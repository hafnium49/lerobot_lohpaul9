#!/usr/bin/env python3
"""
Calculate wrist camera orientation with additional 90° counter-clockwise rotation.
This rotates the camera view 90° CCW around its viewing direction (roll).
"""
...
# Current wrist camera orientation (axis-angle)
current_axis = np.array([-0.999821, -0.018912, -0.000000])
current_angle = 0.479911  # radians

print("="*80)
print("Wrist Camera Rotation Calculation")
print("="*80)

print("\nCurrent orientation (axis-angle):")
print(f"  Axis:  [{current_axis[0]:.6f}, {current_axis[1]:.6f}, {current_axis[2]:.6f}]")
print(f"  Angle: {current_angle:.6f} rad = {np.degrees(current_angle):.2f}°")

# Convert current orientation to rotation matrix
current_rotation = R.from_rotvec(current_axis * current_angle)

print("\nCurrent camera orientation:")
print(f"  Rotation matrix:\n{current_rotation.as_matrix()}")

# Camera local axes after current rotation
camera_forward = current_rotation.apply([0, 0, -1])  # -Z in camera frame
camera_right = current_rotation.apply([1, 0, 0])     # +X in camera frame
camera_up = current_rotation.apply([0, 1, 0])        # +Y in camera frame
...
# Additional 90° counter-clockwise rotation around viewing direction (forward axis)
# This is a roll rotation - rotating the camera view 90° CCW
additional_rotation_angle = np.pi / 2  # 90 degrees in radians
additional_rotation = R.from_rotvec(camera_forward * additional_rotation_angle)

print("\n" + "="*80)
print("Additional 90° CCW Rotation (Roll)")
print("="*80)
...
# Compose rotations: new = additional * current
combined_rotation = additional_rotation * current_rotation
...
# Verify forward direction hasn't changed (should be same as camera_forward)
...
# Verify right and up have rotated 90°
...
if abs(right_rotation_check - 1.0) < 0.01 and abs(up_rotation_check - 1.0) < 0.01:
    print("✓ 90° CCW rotation verified")
else:
    print("⚠ Warning: Rotation may not be exactly 90°")
