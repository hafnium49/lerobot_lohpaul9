...
print("="*80)
print("Finding and Visualizing Square Mounting Surface")
print("="*80)
...
# Find square mounting surface at tip
# Strategy: Look for triangles that are coplanar at the tip with similar Y position
tip_threshold = y_min + 0.025  # Within 25mm of tip
print(f"\nSearching for flat surface at Y < {tip_threshold:.4f} m...")

# First pass: collect all candidate triangles at the tip
candidate_triangles = []
candidate_normals = []
...
for i, (tri_verts, normal) in enumerate(zip(vertices, normals)):
    ...
    # Check: +Y facing normal (outward from mount) - relaxed to 0.50
    if norm[1] > 0.50:
        # Check: at the tip
        tri_y_max = tri_verts[:, 1].max()
        if tri_y_max < tip_threshold:
            candidate_triangles.append(tri_verts)
            candidate_normals.append(norm)

print(f"Found {len(candidate_triangles)} candidate triangles")

# Second pass: filter for triangles at the most outward Y position (the flat mounting surface)
if len(candidate_triangles) > 0:
    candidate_triangles = np.array(candidate_triangles)
    candidate_normals = np.array(candidate_normals)

    # Find the most outward Y position (maximum Y among candidates)
    all_candidate_verts = candidate_triangles.reshape(-1, 3)
    y_surface = all_candidate_verts[:, 1].max()

    print(f"Surface Y position: {y_surface:.4f} m")
    print(f"Filtering for triangles near Y = {y_surface:.4f} m...")

    # Keep only triangles with at least one vertex near the surface Y
    mounting_surface_triangles = []
    mounting_surface_normals = []

    y_tolerance = 0.003  # 3mm tolerance
    for tri_verts, norm in zip(candidate_triangles, candidate_normals):
        tri_y_max = tri_verts[:, 1].max()
        if abs(tri_y_max - y_surface) < y_tolerance:
...
print(f"Found {len(mounting_surface_triangles)} triangles")
...
output_file = "mounting_surface_visualization.png"
